...
## 14. 상태 기반 기능 제한(프로필 수정/친구 요청 차단)은 어디서 검사?
...
0. 서로 다른 목적의 소켓을 사용중. 
공통: 프론트가 withCredentials: true로 쿠키 포함
     gateway가 JWT 검증
     통과 후 사용자 식별(x-user-id 기반)
- daeunki2 : 상태감지용.  내부 API(/internal/presence)에 추가로 x-internal-token 요구
- chanypar : 채팅용.  내부 로직은 주로 x-user-id/query userId로만 처리

1. 경로 맞추기
- daeunki2: `http://api-gateway:8000/internal/presence/...`
 >> api-gateway는 도커 네트워크 안에서 gateway 컨테이너를 찾는 서비스 이름(hostname) 
 >> internal은 브라우저용 공개 API와 서비스간 내부 API를 구분하려고 둔 경로

- chanypar: `http://gateway:8000/presence/...`
>> 아마 제가 전에 쓰던거

2. 토큰 형식 맞추기
- daeunki2 : 내부 API 인증: 헤더 x-internal-token: <PRESENCE_INTERNAL_TOKEN>
- chanypar : x-internal-token은 사용 안 하고 있음. 
>> 제가 push했던 파일에는 저게 없었음

3. 프론트 응답 파싱 맞추기
- 채팅 상태 조회 응답은 `{ status: 'ONLINE' | 'OFFLINE' | 'IN_GAME' }` 형태
- 프론트에서는 객체 전체가 아니라 `response.status` 값을 꺼내서 상태값으로 사용해야 함
>> 이거는 나중에 한번 볼게요

최종 결론
- 상태표시(daeunki2) + 채팅(chanypar)이 같이 안정적으로 동작하려면
  1) 경로
  2) 내부 토큰
  3) 프론트 status 파싱